...
export const joiUrl = Joi.string().uri().default(null).messages({ 'any.invalid': MSG_INCORRECT_URL });
export const joiDate = Joi.date().iso().messages({ 'any.invalid': MSG_INCORRECT_DATE });
export const joiGeoCode = Joi.array().items(Joi.number()).length(2)
